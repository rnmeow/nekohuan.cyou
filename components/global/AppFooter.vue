<template>
  <footer class="py-8">
    <section class="max-w-5xl mx-auto flex flex-col px-8">
      <div class="mt-15 text-sm sm:flex sm:justify-between dark:text-neutral-300">
        <p>
          <FAIcon category="far" icon="copyright" size="sm"/> 郭桓桓 <ClientOnly>{{ outputYear(2023) }}</ClientOnly> / 使用 <FAIcon category="fas" icon="heart" tailwind="text-red-600"/> 和 <NuxtLink to="https://nuxt.com" target="_blank" class="decoration-2 hover:underline">Nuxt 3</NuxtLink> 開發
        </p>
...
export default {
  methods: {
    outputYear(origYear: number): string | number {
      const thisYear: number = new Date().getFullYear()
      if (thisYear !== origYear) return `${origYear}-${thisYear}`
      else if (thisYear === origYear) return origYear
      else return 2023 // hardcoded fallback
    }
  }
}
</script>
